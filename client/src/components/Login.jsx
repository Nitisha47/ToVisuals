...
const Login = () => {
  ...
  const onSubmitHandler = async (e) =>{
    ...
    try {
      if(state === 'Login'){
        //here we need to provide the complete path of the user login api also. And we pass email & password for login too.
        ...
        if(data.success){
           ...
           setShowLogin(false) //to keep the login form hidden.
        }else{
          // react tostify > url copy and install > copy import statement in app.jsx
  ...
  return (
    <div className='fixed top-0 left-0 right-0 bottom-0 z-10 backdrop-blur-sm bg-black/30 flex justify-center items-center'>
      <motion.form onSubmit={onSubmitHandler} initial={{opacity:0.2, y:50}} transition={{duration:0.3}} whileInView={{opacity:1, y:0}} viewport={{once:true}} className='relative bg-white p-10 rounded-xl text-slate-500'>
        <h1 className='text-center text-2xl text-eutral-700 font-medium'>{state}</h1>
